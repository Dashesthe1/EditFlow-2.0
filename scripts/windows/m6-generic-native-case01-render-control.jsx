(function () {
  "use strict";
  var p=app.project,name="__EF2_M6_GENERIC_NATIVE_CASE01_PROOF__",c=null;
  for(var i=1;i<=p.numItems;i++){var x=p.item(i);if(x instanceof CompItem&&x.name===name){c=x;break;}}
  if(!c)throw new Error("M6_GENERIC_CONTROL_COMP_MISSING");
  ...
  var out=new File(Folder.temp.fsName+"/M6_generic_native_case01_control.mp4");
  if(out.exists)out.remove();om.file=out;app.project.renderQueue.render();
  if(!out.exists||out.length<=0)throw new Error("M6_GENERIC_CONTROL_RENDER_EMPTY");
  rq.remove();
}());
