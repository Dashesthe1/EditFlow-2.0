...
try {
  ...
  Write-Host "Scope: P1 deterministic rejection and P2 exact layer-switch/stacking-order readback only. P3/P4/P5 are not claimed."
  Write-Host "Protocol 1.6 owns layer switches and order; protocol 1.1 creates and removes only disposable fixture compositions/layers."
  Write-Host "All 12 declared switch keys are probed independently on a nested-comp AVLayer so switch interactions do not masquerade as capability failure."
  ...
  if (-not $Result.checks.p1_stale_revision_rejected -or -not $Result.checks.p1_stale_revision_fingerprint_unchanged) {
    throw "M3 layer-controls proof did not prove stale-revision rejection without mutation."
  }
  if (-not $Result.checks.p1_empty_switches_rejected -or -not $Result.checks.p1_self_relative_order_rejected) {
    throw "M3 layer-controls proof did not prove deterministic switch/order validation rejection."
  }
  if (-not $Result.checks.p2_all_switches_report_supported -or -not $Result.checks.p2_all_switch_writes_exact) {
    throw "M3 layer-controls proof did not prove every declared protocol 1.6 switch on the nested-comp AVLayer fixture."
  }
  if (-not $Result.checks.p2_locked_move_end_exact -or -not $Result.checks.p2_move_before_exact -or -not $Result.checks.p2_move_after_exact -or -not $Result.checks.p2_move_beginning_exact) {
    throw "M3 layer-controls proof did not prove exact locked-layer stacking-order behavior."
  }
