...
try {
  ...
  Write-Host "EditFlow 2.0 M3 layer-controls real-AE P1/P2 proof through authenticated CEP transport"
  ...
  Write-Host "Scope: P1 deterministic pre-mutation rejection and P2 exact layer-switch/applicability readback only. P3/P4/P5 are not claimed."
  Write-Host "Protocol 1.6 owns typed layer switches/readback; protocol 1.1 creates only disposable fixture objects."
  Write-Host "The proof exercises all eleven declared protocol-1.6 switches, explicit locked-layer rejection, idempotency, atomic unlock/restore, and exact baseline project cleanup."
  ...
  if ($Result.cleanupComplete -ne $true) {
    ...
    throw "M3 layer-controls P1/P2 result was produced without successful temporary-project cleanup."
  }
  if (-not $Result.ok) {
    $ResultJson | Write-Host
  ...
  if (-not $Result.checks.p1_unknown_switch_rejected -or -not $Result.checks.p1_unknown_fingerprint_unchanged) {
    throw "M3 layer-controls proof did not prove unknown-switch rejection without project mutation."
  }
  if (-not $Result.checks.p1_stale_revision_rejected -or -not $Result.checks.p1_locked_change_rejected -or -not $Result.checks.p1_locked_fingerprint_unchanged) {
    throw "M3 layer-controls proof did not prove stale-state and locked-layer rejection without mutation."
  }
  if (-not $Result.checks.p2_full_patch_applied -or -not $Result.checks.p2_full_patch_exact -or -not $Result.checks.p2_repeat_patch_no_op) {
    throw "M3 layer-controls proof did not prove exact all-switch mutation/readback and idempotency."
  }
  if (-not $Result.checks.p2_atomic_unlock_restore_applied -or -not $Result.checks.p2_final_readback_matches_baseline) {
    throw "M3 layer-controls proof did not prove explicit atomic unlock and exact fixture restoration."
  }
  if (-not $Result.checks.cleanup_fingerprint_restored) {
    throw "M3 layer-controls proof did not restore the exact pre-proof project fingerprint."
  }
