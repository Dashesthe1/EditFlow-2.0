...
$NodeProof = Join-Path $RepoRoot "scripts\m5-roto-brush-repair-live-proof.mjs"
...
$RepairVisualScript = Join-Path $RepoRoot "packages\adapters\ae-cep\runtime\editgpt_roto_brush_repair_visual_driver.py"
$VisualWorkdir = Join-Path $RepoRoot "packages\adapters\ae-cep\runtime"
$ArtifactDir = Join-Path $RepoRoot "proofs\artifacts\m5-roto-brush-repair-live-proof"
...
$Message = "M5 Roto Brush repair retained proof did not complete."
...
try {
  foreach ($Required in @($AfterFxPath,$SourcePath,$EnterScript,$RestoreScript,$PreflightRunner,$FixtureScript,$ReadbackScript,$ToolSelectScript,$NodeProof,$PythonPath,$SeedVisualScript,$RepairVisualScript)) {
    if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) { throw "Required M5 repair live-proof file missing: $Required" }
  }
  $Running = @(Get-Process -Name "AfterFX" -ErrorAction SilentlyContinue)
  if ($Running.Count -ne 1) { throw "M5 repair live proof requires exactly one already-running After Effects process; found $($Running.Count)." }
  ...
  if (Test-Path $StatePath -PathType Leaf) {
    ...
    $PriorRestore = Wait-JsonMarker $RestoreMarker $TimeoutSeconds "M5 prior isolation recovery"
    if ($PriorRestore.version -ne "M5_ROTO_BRUSH_ISOLATION_V1" -or $PriorRestore.ok -ne $true) { throw "M5 prior isolation recovery refused: $($PriorRestore.failure)" }
  ...
  try {
    & node $NodeProof `
      ...
      --repair-visual-script $RepairVisualScript `
      --visual-workdir $VisualWorkdir `
      --evidence-dir $EvidenceDir `
      --timeout-ms ($TimeoutSeconds * 1000)
    $NodeExit = $LASTEXITCODE
  } finally { Pop-Location }
  if (-not (Test-Path $LiveResultPath -PathType Leaf)) { throw "M5 repair live controller exited without a result (exit $NodeExit)." }
  $Live = Get-Content $LiveResultPath -Raw | ConvertFrom-Json
  if ($Live.proofId -ne "M5_ROTO_BRUSH_REPAIR_STROKE_REAL_AE_V1") { throw "Unexpected M5 repair live proof result identity." }
  if ($NodeExit -ne 0 -or $Live.ok -ne $true) {
    $Reason = if ($Live.failure) { [string]$Live.failure } else { "M5 guarded live repair controller failed." }
    throw $Reason
  }
} catch {
  ...
  if ($null -eq $PrimaryFailure -and $null -eq $RestoreFailure -and $Live -and $Live.ok -eq $true -and $RestoreAttempted -and $Restore.ok -eq $true -and $SameAeProcess) {
    $Classification = "PASS"
    $Message = "M5 live Roto Brush repair passed with a known visible background defect, same-path guarded foreground correction, native stroke readback, bounded latency, exact restore, and one warm AE process."
  } else {
    if ($null -ne $RestoreFailure) { $Message = $RestoreFailure }
    elseif ($null -ne $PrimaryFailure) { $Message = $PrimaryFailure }
    elseif (-not $SameAeProcess) { $Message = "After Effects process identity changed during the M5 repair proof." }
    else { $Message = "M5 repair proof did not satisfy all retained gates." }
  }
  $Result = [ordered]@{
    proofId = "M5_ROTO_BRUSH_REPAIR_STROKE_RETAINED_REAL_AE_V1"
