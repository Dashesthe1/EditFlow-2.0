(function () {
  ...
  function dispatchRaw(protocolVersion, capabilityId, command, payload) {
    ...
    var request = {
      ...
      requestId: "m4-sequence-p5-stage1-" + requestCounter,
      transactionId: "M4_SEGMENTATION_SEQUENCE_P5_STAGE1",
      operationId: "M4_SEGMENTATION_SEQUENCE_P5_STAGE1_" + requestCounter,
      capabilityId: capabilityId,
      command: command,
      payload: payload,
      expectedHostProjectRevision: expectedRevision,
      readbackProfile: "M4_SEGMENTATION_SEQUENCE_P5_STAGE1"
    };
  ...
  var baselineItems = -1;
  var baselineRenderItems = -1;
  var baselineFile = null;
  try {
    if (!app.project) throw new Error("No After Effects project is open");
    if (!lifecycleProjectFile.exists) throw new Error("Lifecycle project copy is missing: " + lifecycleProjectFile.fsName);
    app.open(lifecycleProjectFile);
    if (!app.project || !app.project.file || app.project.file.fsName !== lifecycleProjectFile.fsName) throw new Error("Lifecycle project copy did not open exactly");
    baselineItems = app.project.numItems;
    baselineRenderItems = app.project.renderQueue.numItems;
    baselineFile = app.project.file.fsName;
    ...
    dispatch("1.1.0", "ae.comp.create", "comp.create", {
      stableId: "M4_TRANSFER_SEQUENCE_COMP", name: "M4 Segmentation Sequence P5 Lifecycle Proof",
    ...
    lifecycleSaved = lifecycleProjectFile.exists && app.project.file && app.project.file.fsName === lifecycleProjectFile.fsName;
    checks.lifecycle_project_saved = lifecycleSaved;
    checks.lifecycle_state_retained = !!findItem("M4_TRANSFER_SEQUENCE_COMP") && !!findItem(plan.plan.importItemStableId) && !!findLayer(proofComp, plan.plan.matteLayerStableId);
  } catch (error) {
  ...
  var ok = failure === null && allChecks && lifecycleSaved;
  writeJson(resultFile, {
    schemaVersion: 1,
    proofId: "M4_SEGMENTATION_SEQUENCE_P5_STAGE1",
    ok: ok,
    mutationStarted: mutationStarted,
    lifecycleSaved: lifecycleSaved,
    ...
    after: { itemCount: app.project ? app.project.numItems : null, renderQueueCount: app.project ? app.project.renderQueue.numItems : null, projectFile: afterFile },
    artifacts: { plan: planFile.fsName, lifecycleProject: lifecycleProjectFile.fsName, baselineFrames: [baselineFiles[0].fsName, baselineFiles[1].fsName, baselineFiles[2].fsName], reviewFrames: [reviewFiles[0].fsName, reviewFiles[1].fsName, reviewFiles[2].fsName] }
  });
}());
