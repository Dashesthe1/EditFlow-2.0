...
$TempPath = Join-Path $PSScriptRoot ("run-m3-temporal-interpolation-self-hosted-generated-" + [Guid]::NewGuid().ToString("N") + ".ps1")
$ProofArtifactDir = Join-Path $RepoRoot "proofs\artifacts\m3-temporal-interpolation-p1-p2"
...
function Copy-CepFailureDiagnostics {
  ...
  $ManifestLines = @(
    "EditFlow M3 temporal-interpolation CEP failure diagnostics",
