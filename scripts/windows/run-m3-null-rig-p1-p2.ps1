...
try {
  ...
  Write-Host "EditFlow 2.0 M3 null-rig real-AE P1/P2 proof through authenticated CEP transport"
  ...
  Write-Host "Scope: P1 deterministic rejection and P2 exact managed-null/relationship-topology readback only. P3/P4/P5 are not claimed."
  Write-Host "Protocol 1.5 owns null lifecycle/readback; accepted protocol 1.4 owns explicit child attach/detach; protocol 1.1 creates only disposable fixture objects."
  ...
  if ($Result.cleanupComplete -ne $true) {
    ...
    throw "M3 null-rig P1/P2 result was produced without successful temporary-project cleanup."
  }
  if (-not $Result.ok) {
    $ResultJson | Write-Host
  ...
  if (-not $Result.checks.p1_child_protected_remove_rejected -or -not $Result.checks.p1_child_protected_fingerprint_unchanged) {
    throw "M3 null-rig proof did not prove child-protected removal was rejected without project mutation."
  }
  if (-not $Result.checks.p2_create_exact_identity -or -not $Result.checks.p2_topology_reports_child) {
    throw "M3 null-rig proof did not show exact managed-null identity and attached-child topology readback."
  }
  if (-not $Result.checks.p2_topology_empty_after_detach -or -not $Result.checks.p2_project_snapshot_rig_absent) {
    throw "M3 null-rig proof did not show explicit detach followed by exact null-rig removal."
  }
  if (-not $Result.checks.cleanup_fingerprint_restored) {
    throw "M3 null-rig proof did not restore the exact pre-proof project fingerprint."
  }
