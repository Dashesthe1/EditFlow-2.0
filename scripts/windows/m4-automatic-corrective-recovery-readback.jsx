(function () {
  ...
  var out = new File(root.fsName + "/proofs/artifacts/m4-automatic-corrective-recovery-readback.json");
  ...
  try{
    ...
    var setup=readJson(setupFile);if(!setup.ok)throw new Error("Automatic corrective setup not accepted.");
    var comp=app.project.itemByID?app.project.itemByID(setup.fixtureCompId):null;if(!comp||!(comp instanceof CompItem))throw new Error("Automatic corrective fixture missing.");
    var layer=app.project.layerByID?app.project.layerByID(setup.layerId):comp.layer(1);if(!layer||layer.containingComp!==comp)throw new Error("Automatic corrective layer mismatch.");
    if(!loader.exists)throw new Error("Protocol 2.4 loader missing.");$.evalFile(loader);
    var request={protocolVersion:"2.1.0",requestId:"M4_ACR_READBACK",transactionId:"M4_ACR_READBACK",operationId:"M4_ACR_READBACK",capabilityId:"ae.tracker.readback",command:"tracker.readback",expectedHostProjectRevision:null,payload:{comp:{hostId:comp.id},layer:{hostId:layer.id}},readbackProfile:"M4_AUTOMATIC_CORRECTIVE_RECOVERY_VERIFY"};
    ...
    if(!point)throw new Error("Expected one automatically repaired tracker point.");
