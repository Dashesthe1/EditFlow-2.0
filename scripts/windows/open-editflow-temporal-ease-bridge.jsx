/* EditFlow 2.0 protocol 1.8 self-hosted panel bootstrap diagnostic.
 *
 * This fixed diagnostic script loads the installed v1.8 host chain directly
 ...
 * the production-equivalent proof runner. It performs no project mutation and
 * evaluates no caller-supplied script text.
 */
(function () {
  "use strict";
  ...
  var installedHost = new File(Folder.userData.fsName + "/Adobe/CEP/extensions/com.editflow2.bridge/host/editflow_host_current_v18.jsx");
  appendEvidence("HOST_PATH", "exists=" + installedHost.exists + ";path=" + installedHost.fsName);
  appendEvidence(
    "HOST_STATE_BEFORE",
    "v18=" + String($.global.EditFlow2_HOST_PROTOCOL_18 === true)
  ...
  try {
    if (!installedHost.exists) throw new Error("Installed EditFlow protocol 1.8 host loader is missing.");
    ...
    appendEvidence(
      ...
      "result=" + String(result)
        ...
        + ";v16=" + String($.global.EditFlow2_HOST_PROTOCOL_16 === true)
        + ";dispatch=" + typeof $.global.EditFlow2_dispatch
        + ";moduleLoadError=" + String($.global.EditFlow2_M3_TEMPORAL_EASE_LOAD_ERROR || "")
    );
  } catch (hostLoadError) {
    appendEvidence("HOST_LOAD_ERROR", errorDetail(hostLoadError));
  }

  $.global.EditFlow2_temporalEaseBridgeOpenAttempts = 0;
  $.global.EditFlow2_temporalEaseOpenBridge = function () {
    var menuName = "EditFlow 2.0 Bridge";
    var maxAttempts = 120;
    var retryDelayMs = 500;
    var attempts = Number($.global.EditFlow2_temporalEaseBridgeOpenAttempts || 0) + 1;
    $.global.EditFlow2_temporalEaseBridgeOpenAttempts = attempts;
    ...
    try {
      ...
      if (typeof commandId === "number" && commandId > 0) {
        ...
        $.global.EditFlow2_temporalEaseBridgeOpened = true;
        return;
      }
    } catch (error) {
    ...
    if (attempts < maxAttempts) {
      var taskId = app.scheduleTask("$.global.EditFlow2_temporalEaseOpenBridge()", retryDelayMs, false);
      appendEvidence("RETRY_SCHEDULED", "attempt=" + attempts + ";taskId=" + taskId);
    } else {
      appendEvidence("RETRY_EXHAUSTED", "attempts=" + attempts);
      $.global.EditFlow2_temporalEaseBridgeOpenFailed = true;
    }
  };

  appendEvidence("INITIAL_ATTEMPT_DIRECT", "attempt=1");
  $.global.EditFlow2_temporalEaseOpenBridge();
}());
