...
$TempPath = Join-Path $PSScriptRoot ("run-m3-marker-motion-self-hosted-generated-" + [Guid]::NewGuid().ToString("N") + ".ps1")
$ProofArtifactDir = Join-Path $RepoRoot "proofs\artifacts\m3-marker-motion-p1-p2"
...
function Copy-CepFailureDiagnostics {
  ...
  $Manifest = @(
    "EditFlow M3 marker-motion CEP failure diagnostics",
...
# Protocol 2.0 is intentionally not advertised by the normal installer until its
# real-AE proof is accepted. This block changes only the isolated proof installation:
# it copies the v20 host modules, extends the installed runtime config to 2.0, and
# teaches the installed panel bootstrap to choose current_v20. Repository defaults
# and ordinary EditFlow sessions remain on accepted protocol 1.9.
$PreviewInstallBlock = @'
& $Installer
...
foreach ($RequiredSource in @($MarkerMotionHostSource, $V20LoaderSource)) {
  if (-not (Test-Path $RequiredSource -PathType Leaf)) { throw "Protocol 2.0 proof host source is missing: $RequiredSource" }
}
Copy-Item $MarkerMotionHostSource (Join-Path $InstalledHost "editflow_host_m3_marker_motion.jsx") -Force
Copy-Item $V20LoaderSource (Join-Path $InstalledHost "editflow_host_current_v20.jsx") -Force
...
Write-Host "Isolated protocol 2.0 marker-motion preview installed for this proof only; normal installer defaults remain accepted protocol 1.9."
'@

$MarkerMotion = $Template
$MarkerMotion = $MarkerMotion.Replace('scripts\windows\run-m3-mask-p1-p2.ps1', 'scripts\windows\run-m3-marker-motion-p1-p2.ps1')
$MarkerMotion = $MarkerMotion.Replace('proofs\artifacts\m3-mask-p1-p2', 'proofs\artifacts\m3-marker-motion-p1-p2')
$MarkerMotion = $MarkerMotion.Replace('The M3 mask P1/P2 acceptance runner is missing', 'The M3 marker-motion P1/P2 acceptance runner is missing')
...
$MarkerMotion = $MarkerMotion.Replace('& $Installer', $PreviewInstallBlock)
...
try {
  ...
  if ($EffectiveLogLevel -ne "6") {
    throw "Unable to arm CEP 12 verbose logging for the isolated marker-motion proof. Registry readback was '$EffectiveLogLevel'."
  }
  Write-Host "CEP 12 LogLevel registry readback before AE launch: $EffectiveLogLevel"
  Write-Host "Marker-motion proof mode: isolated authenticated protocol 2.0 preview; accepted installation remains 1.9 outside this proof."
  ...
  for ($Attempt = 1; $Attempt -le $MaxPanelRegistrationAttempts; $Attempt++) {
    ...
    try {
      ...
      if ($LASTEXITCODE -ne 0) { throw "Marker-motion generated self-hosted runner exited with code $LASTEXITCODE." }
      $Completed = $true
      break
    } catch {
      $AttemptError = $_
    ...
    if ($Attempt -lt $MaxPanelRegistrationAttempts -and $RetryableRegistrationFailure) {
      ...
      if ($RemainingAfterFx.Count -ne 0) {
        ...
        throw "Marker-motion panel-registration retry refused because the failed zero-command attempt did not return to a zero-After-Effects baseline."
      }
      Retain-PanelRetryEvidence -Attempt $Attempt
      Write-Host ("Authenticated CEP panel registration timed out before any marker-motion command on attempt " + $Attempt + "; retrying one fresh isolated AE launch from the verified zero-process baseline.")
  ...
  if (-not $Completed) { throw "Marker-motion self-hosted runner exhausted its bounded panel-registration attempts." }
} catch {
  Copy-CepFailureDiagnostics -Destination $ProofArtifactDir
  throw
} finally {
