...
$ArtifactDir = Join-Path $RepoRoot "proofs\artifacts\m3-temporal-ease-p1-p2"
...
function Resolve-RunningAfterFx {
  ...
  if ($Running.Count -eq 0) {
    throw "Adobe After Effects is not running. The M3 temporal-ease P1/P2 wrapper requires the isolated self-hosted AE process to be running first."
  }
  ...
  if ($RunningPaths.Count -gt 1) { throw "Multiple After Effects installations are running; the bounded M3 temporal-ease proof requires exactly one isolated target." }
...
try {
  ...
  $Cli = Join-Path $RepoRoot ".tmp\runtime\apps\desktop-host\src\m3-temporal-ease-p1-p2-cli.js"
  if (-not (Test-Path $Cli -PathType Leaf)) { throw "Compiled M3 temporal-ease P1/P2 CLI not found: $Cli" }

  Write-Host "EditFlow 2.0 M3 temporal-ease real-AE P1/P2 proof through authenticated CEP transport"
  ...
  while (-not $NodeProcess.HasExited) {
    ...
    if ($CleanupComplete) {
      if ($null -eq $ResultSeenAt) {
        ...
        Write-Warning "M3 temporal-ease P1/P2 proof and cleanup are complete but Node did not exit; terminating only the completed proof runtime."
    ...
    if ((Get-Date) -ge $HardDeadline) {
      Stop-Process -Id $NodeProcess.Id -Force -ErrorAction SilentlyContinue
      $NodeProcess.WaitForExit()
      throw "M3 temporal-ease P1/P2 acceptance exceeded its hard runtime before cleanup completion."
    }
  ...
  if (-not (Test-Path $ResultPath -PathType Leaf)) {
    throw "M3 temporal-ease P1/P2 acceptance exited without a proof artifact (exit code $($NodeProcess.ExitCode))."
  }
  ...
  if ($Result.cleanupComplete -ne $true) {
    ...
    throw "M3 temporal-ease P1/P2 result was produced without successful temporary-project cleanup."
  }
  if (-not $Result.ok) {
    $ResultJson | Write-Host
    throw "M3 temporal-ease P1/P2 real-AE proof did not pass all bounded checks."
  }
  if (-not $Result.proofLevels.P1_validation_rejection -or -not $Result.proofLevels.P2_structural_readback) {
    throw "M3 temporal-ease result did not truthfully assert both P1 and P2."
  }
  if ($Result.proofLevels.P3_visual_proof -or $Result.proofLevels.P4_failure_injection_rollback -or $Result.proofLevels.P5_save_reopen_reconnect_transfer) {
    throw "M3 temporal-ease P1/P2 harness must not claim P3, P4, or P5."
  }

  $RequiredP1 = @(
    "p1_bad_key_rejected",
    "p1_bad_path_rejected",
    "p1_requires_bezier_rejected",
    "p1_bad_cardinality_rejected",
    "p1_bad_influence_rejected",
    "p1_requires_manual_bezier_rejected",
    "p1_stale_revision_rejected"
  )
  foreach ($CheckName in $RequiredP1) {
    if ($Result.checks.$CheckName -ne $true) { throw "M3 temporal-ease P1 proof is missing required rejection check: $CheckName" }
  }

  $RequiredP2 = @(
    "p2_opacity_scalar_readback_exact",
    "p2_opacity_scalar_readback_cardinality_exact",
    "p2_opacity_scalar_readback_manual_bezier",
    "p2_opacity_scalar_key_identity_exact",
    "p2_scalar_exact_noop",
    "p2_scalar_noop_revision_unchanged",
    "p2_scale_twod_readback_exact",
    "p2_scale_twod_readback_cardinality_exact",
    "p2_scale_twod_readback_manual_bezier",
    "p2_scale_twod_key_identity_exact"
  )
  foreach ($CheckName in $RequiredP2) {
    if ($Result.checks.$CheckName -ne $true) { throw "M3 temporal-ease P2 proof is missing required structural check: $CheckName" }
  }

  if (-not $Result.checks.cleanup_fingerprint_restored -or -not $Result.checks.cleanup_item_count_restored) {
    throw "M3 temporal-ease proof did not restore the exact pre-proof project baseline."
  }

  Write-Host ("M3 temporal-ease P1/P2 proof status: " + $Result.status)
