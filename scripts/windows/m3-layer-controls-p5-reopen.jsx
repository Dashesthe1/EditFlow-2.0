/* EditFlow 2.0 M3 layer-controls P5 proof-only save/reopen transfer step.
 * RUN ONLY on the isolated self-hosted Windows AE runner with
 * EDITFLOW_M3_LAYER_CONTROLS_P5_PROOF=1. The project path is fixed inside the
...
(function () {
  ...
  var hostScript = new File(repoRoot.fsName + "/packages/adapters/ae-cep/host/editflow_host_current_v16.jsx");
  ...
  function writeMarker(value) {
    ...
    if (!markerFile.open("w")) throw new Error("Unable to open M3 layer-controls P5 reopen marker: " + markerFile.fsName);
    try { markerFile.write(stringify(value)); } finally { markerFile.close(); }
  }

  var payload = {
    proofId: "M3_LAYER_CONTROLS_P5_REOPEN",
    ok: false,
    error: null,
    projectPath: null,
    itemCount: null,
  ...
  try {
    if ($.getenv(PROOF_ENV) !== "1") throw new Error("REFUSED: M3 layer-controls P5 reopen requires the isolated proof environment.");
    if (!app.project) throw new Error("M3 layer-controls P5 reopen requires an open After Effects project.");
    if (!projectFile.exists) throw new Error("M3 layer-controls P5 saved project artifact does not exist: " + projectFile.fsName);
    if (!app.project.file || !samePath(app.project.file.fsName, projectFile.fsName)) {
      throw new Error("M3 layer-controls P5 reopen refuses to close a project other than the fixed runner-owned saved proof project.");
    }
    if (!hostScript.exists) throw new Error("EditFlow protocol 1.6 host loader is missing: " + hostScript.fsName);

    var closed = app.project.close(CloseOptions.DO_NOT_SAVE_CHANGES);
    if (closed === false) throw new Error("M3 layer-controls P5 reopen could not close the saved disposable proof project.");
    app.open(projectFile);
    if (!app.project || !app.project.file || !samePath(app.project.file.fsName, projectFile.fsName)) {
      throw new Error("M3 layer-controls P5 reopen did not reopen the fixed saved project.");
    }

    /* Force a fresh dispatcher registration from the additive protocol-1.6 loader
     * rather than treating the pre-save global function as transfer evidence. */
    try { $.global.EditFlow2_dispatch = undefined; } catch (_) {}
    $.evalFile(hostScript);
    if (typeof $.global.EditFlow2_dispatch !== "function") {
      throw new Error("M3 layer-controls P5 protocol 1.6 dispatcher did not register after reopen.");
    }
