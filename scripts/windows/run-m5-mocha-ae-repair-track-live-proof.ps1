...
$SeekScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-seek-frame-one.ps1'
$TrackScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-guarded-track-range.ps1'
$CloseProofScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-close-proof-session.ps1'
$EssentialsScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-switch-workspace.ps1'
$RepairTrackScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-repair-track.ps1'
$RepairCompareScript=Join-Path $RepoRoot 'scripts\m5-mocha-ae-verify-repair.py'
$PrepareAdjustTrackScript=Join-Path $RepoRoot 'scripts\windows\m5-mocha-ae-prepare-adjusttrack.ps1'
...
$VerifyMarker=Join-Path $env:TEMP 'EditFlow2-m5-mocha-ae-isolation-verify.json'
...
try{
  ...
  if($LASTEXITCODE -eq 0){
    $panelTarget=[IO.File]::ReadAllText($panelLaunchJson,[Text.Encoding]::UTF8)|ConvertFrom-Json; $pre=[pscustomobject]@{image=$panelPrePng;targetPath=$panelLaunchJson;target=$panelTarget}
  }else{
    $tabJson=Join-Path $ArtifactDir 'effect-controls-tab-pre.target.json'; python $TabTargetScript --image $panelPrePng --result $tabJson | Out-Null
    if($LASTEXITCODE -eq 0){
      $ControlsTabActivationMode+='>GUARDED_TAB_CLICK'; $tabClickResult=Join-Path $ArtifactDir 'effect-controls-tab-click.json'; $tabSw=[Diagnostics.Stopwatch]::StartNew()
      powershell.exe -NoProfile -ExecutionPolicy Bypass -File $TabClickScript -AfterFxPid $BaselineAePids[0] -TargetJson $tabJson -ResultPath $tabClickResult | Out-Null; $tabSw.Stop(); $ControlsTabClickMs=[Math]::Round($tabSw.Elapsed.TotalMilliseconds,3)
      if($LASTEXITCODE -ne 0){throw 'Guarded Effect Controls tab activation failed.'}; Start-Sleep -Milliseconds 120; $pre=Capture-And-Target 'launch-pre'
    }else{
      $ControlsTabActivationMode+='>F3_FALLBACK'; $activateSw=[Diagnostics.Stopwatch]::StartNew()
      powershell.exe -NoProfile -ExecutionPolicy Bypass -File $ActivateControlsScript -AfterFxPid $BaselineAePids[0] -ResultPath $activateResult | Out-Null; $activateSw.Stop(); $ControlsActivationMs=[Math]::Round($ControlsActivationMs+$activateSw.Elapsed.TotalMilliseconds,3)
      if($LASTEXITCODE -ne 0){throw 'F3 Effect Controls fallback activation failed.'}; Start-Sleep -Milliseconds 120; $pre=Capture-And-Target 'launch-pre'
    }
  }
  ...
  $allowedInitialTitles=@('Mocha AE','Registration','Welcome to Mocha AE','Thank you from Boris FX')
  if(-not($allowedInitialTitles -contains $title)){throw "Mocha process window title was not an allowed exact startup title: $title"}
  ...
  $startupPromptResult=Join-Path $ArtifactDir 'mocha-startup-prompts.json'
  $startupPromptSw=[Diagnostics.Stopwatch]::StartNew()
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $StartupPromptScript -MochaPid $LaunchedMocha.Id -ResultPath $startupPromptResult | Out-Null
  $startupPromptSw.Stop(); $StartupPromptMs=[Math]::Round($startupPromptSw.Elapsed.TotalMilliseconds,3)
  if($LASTEXITCODE -ne 0){throw 'Mocha startup-prompt handling failed.'}
  $StartupPrompts=[IO.File]::ReadAllText($startupPromptResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($StartupPrompts.ok -ne $true){throw 'Mocha startup-prompt handling did not return an unobstructed session.'}
  ...
  $workspacePreResult=Join-Path $ArtifactDir 'workspace-essentials-pre.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $EssentialsScript -MochaPid $LaunchedMocha.Id -MochaMainWindowHandle $MochaIdentity.mainWindowHandle -ResultPath $workspacePreResult | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha Essentials workspace preflight failed.'}
  $WorkspacePre=[IO.File]::ReadAllText($workspacePreResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($WorkspacePre.ok -ne $true){throw 'Mocha Essentials workspace preflight did not verify.'}
  $seekResult=Join-Path $ArtifactDir 'seek-frame-one.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $SeekScript -MochaPid $LaunchedMocha.Id -ResultPath $seekResult | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha bounded seek to frame 1 failed.'}
  $Seek=[IO.File]::ReadAllText($seekResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($Seek.ok -ne $true -or $Seek.beforeFrame -ne 0 -or $Seek.afterFrame -ne 1){throw 'Mocha did not reach exact planar-tracking seed frame 1.'}
  ...
  $trackResult=Join-Path $ArtifactDir 'planar-tracking-action.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $TrackScript -MochaPid $LaunchedMocha.Id -ResultPath $trackResult | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha bounded bidirectional planar tracking failed.'}
  $Track=[IO.File]::ReadAllText($trackResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($Track.ok -ne $true -or $Track.trackedRangeVerified -ne $true -or $Track.forwardEndpointVerified -ne $true -or $Track.backwardEndpointVerified -ne $true){throw 'Mocha tracked range was not independently verified in both directions.'}
  $PlanarTrackingVerified=$true
  $menuProbeResult=Join-Path $ArtifactDir 'track-menu-discovery.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $PrepareAdjustTrackScript -MochaPid $LaunchedMocha.Id -MochaMainWindowHandle $MochaIdentity.mainWindowHandle -ResultPath $menuProbeResult | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha Track-menu discovery failed.'}
  $RepairDiscovery=[IO.File]::ReadAllText($menuProbeResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($RepairDiscovery.ok -ne $true -or $RepairDiscovery.adjustTrackSelected -ne $true -or $RepairDiscovery.setPointsVerified -ne $true){throw 'Mocha AdjustTrack discovery did not verify selected tab plus inline Set Points transaction.'}
  $CorrectionFrame=[int]$Track.backward.afterTrack.frame
  if($CorrectionFrame -ne 0){throw "Retained repair proof expected correction frame 0 after bounded backward solve; got $CorrectionFrame."}
  $repairDir=Join-Path $ArtifactDir 'repair-track';New-Item -ItemType Directory -Force -Path $repairDir|Out-Null
  $repairActionResult=Join-Path $repairDir 'repair-action.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $RepairTrackScript -MochaPid $LaunchedMocha.Id -MochaMainWindowHandle $MochaIdentity.mainWindowHandle -CorrectionFrame $CorrectionFrame -ArtifactDir $repairDir -CaptureScript $CaptureScript -ResultPath $repairActionResult -DefectNudges 8 | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha AdjustTrack semantic defect/repair action failed.'}
  $RepairAction=[IO.File]::ReadAllText($repairActionResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($RepairAction.ok -ne $true -or $RepairAction.correctionFrame -ne $CorrectionFrame -or $RepairAction.adjustTrackSelected -ne $true){throw 'Mocha repair action readback did not bind the expected correction frame/session.'}
  $repairVerifyResult=Join-Path $repairDir 'repair-visual-verification.json'
  python $RepairCompareScript --baseline $RepairAction.baselineImage --defect $RepairAction.defectImage --repaired $RepairAction.repairedImage --action-json $repairActionResult --result $repairVerifyResult | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha repair visual verification did not prove a visible defect followed by restoration.'}
  $RepairVerify=[IO.File]::ReadAllText($repairVerifyResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  if($RepairVerify.ok -ne $true -or $RepairVerify.defect.visible -ne $true -or $RepairVerify.repairedResidual.restored -ne $true){throw 'Mocha repair independent verification gates did not pass.'}
  $RepairTrackVerified=$true
  $workspacePostResult=Join-Path $ArtifactDir 'workspace-essentials-post.json'
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File $EssentialsScript -MochaPid $LaunchedMocha.Id -MochaMainWindowHandle $MochaIdentity.mainWindowHandle -ResultPath $workspacePostResult -WorkspaceOnly | Out-Null
  if($LASTEXITCODE -ne 0){throw 'Mocha Essentials workspace restore failed after AdjustTrack discovery.'}
  $WorkspacePost=[IO.File]::ReadAllText($workspacePostResult,[Text.Encoding]::UTF8)|ConvertFrom-Json
  $LaunchVerified=$true
    ...
    if($remaining.Count -eq 1){
      try{
        ...
        if($rpath -and $rpath -match '(?i)\\mocha4ae_adobe\.exe$'){
          try{if(Test-Path $EssentialsScript){powershell.exe -NoProfile -ExecutionPolicy Bypass -File $EssentialsScript -MochaPid $rp.Id -MochaMainWindowHandle $rp.MainWindowHandle -ResultPath (Join-Path $ArtifactDir 'workspace-essentials-finally.json') -WorkspaceOnly | Out-Null}}catch{}
  ...
  if($MutationStarted -eq $true){
    ...
    try{
      ...
      $run=Invoke-AeTimed $VerifyScript $VerifyMarker 'Mocha settled restore verify'; $SettledRestore=$run.value; $SettledRestoreMs=$run.roundtripMs
  ...
  if($null -eq $PrimaryFailure -and $null -eq $RestoreFailure -and $LaunchVerified -and $PlanarRegionVerified -and $PlanarTrackingVerified -and $RepairTrackVerified -and $CleanupComplete -and $MaxWarmMs -le 3000){
    $Classification='PASS'; $Message='Mocha AE created a bounded AdjustTrack defect, applied the exact inverse repair, independently verified visible restoration, and restored the exact AE project.'
  ...
  $solver=@(); if($Track){$solver=@($Track.forward.trackActionMs,$Track.backward.trackActionMs)|Where-Object{$null -ne $_}}
  $MaxTrackMs=if($solver.Count){[Math]::Round(($solver|Measure-Object -Maximum).Maximum,3)}else{$null}
  $Result=[ordered]@{
  ...
  if($ProofMutexAcquired){try{$ProofMutex.ReleaseMutex()}catch{};$ProofMutexAcquired=$false}
  if($null -ne $ProofMutex){$ProofMutex.Dispose()}
}
if($Classification -ne 'PASS'){Write-Error $Message;exit 1}
Write-Host $Message
Write-Host ("Result: "+$ResultPath)
Write-Host ("Warm max: "+$MaxWarmMs+" ms; max Mocha one-frame solve: "+$MaxTrackMs+" ms; click-to-Mocha-window: "+$ClickToWindowMs+" ms")
