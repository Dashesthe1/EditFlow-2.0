...
$dir=Join-Path $root "proofs\artifacts\m3-human-parity-exit-gate-p3"
...
try{
  New-Item -ItemType Directory -Force -Path $dir|Out-Null;foreach($p in @($core,$result,$ready,$stdout,$stderr)){if(Test-Path $p){Remove-Item $p -Force}}
  ...
  try{if(-not(Test-Path (Join-Path $root "node_modules"))){npm install;if($LASTEXITCODE -ne 0){throw "npm install failed"}};npm run build:test-runtime;if($LASTEXITCODE -ne 0){throw "runtime build failed"}
    ...
    $d=(Get-Date).AddSeconds(5);while((Get-Date)-lt $d -and -not(Test-Path $ready)){$node.Refresh();if($node.HasExited){break};Start-Sleep -Milliseconds 50};if(-not(Test-Path $ready)){throw "Exit-gate broker did not become ready"}
    ...
    if(-not $node.WaitForExit($ProofTimeoutSeconds*1000)){Stop-Process -Id $node.Id -Force -ErrorAction SilentlyContinue;throw "Exit-gate core exceeded $ProofTimeoutSeconds seconds"};if($node.ExitCode -ne 0){if(Test-Path $stderr){Get-Content $stderr -Tail 80|Write-Host};if(Test-Path $core){Get-Content $core -Raw|Write-Host};throw "Exit-gate core failed with $($node.ExitCode)"}
  ...
  $final=[ordered]@{proofId="M3_HUMAN_PARITY_EXIT_GATE_P3_WARM";status=if($ok){"VISUAL_REVIEW_REQUIRED"}else{"FAILURE"};ok=$ok;elapsedMs=$elapsed;coreElapsedMs=$c.elapsedMs;speedTargetMs=30000;coreSpeedTargetMet=$c.speedTargetMet;visualReviewRequired=$ok;cleanupComplete=$blankAfter -eq "BLANK_UNSAVED";preCleanup=$pre;postCleanup=$post;core=$c};$final|ConvertTo-Json -Depth 30|Out-File $result -Encoding utf8;Get-Content $result -Raw|Write-Host;$exit=if($ok){0}else{1}
}catch{Write-Error $_;if(Test-Path $cleanup){try{$post=Ae $cleanup $cleanupEv "CLEANED ";Write-Host "Failure cleanup: $post"}catch{}};$exit=1}finally{if($node){try{$node.Refresh();if(-not $node.HasExited){Stop-Process -Id $node.Id -Force -ErrorAction SilentlyContinue}}catch{}}}
exit $exit
