/* Removes only stale M3 marker-motion proof-owned AE objects. */
(function () {
  "use strict";
  var prefix = "M3_MARKER_MOTION_FAST_";
  var stablePrefix = "[[EDITFLOW2_STABLE:";
  var stableSuffix = "]]";
  var evidence = new File(Folder.temp.fsName + "/EditFlow2-m3-marker-motion-stale-cleanup.txt");
  ...
  function owned(item) {
    try {
      var id = stableId(item.comment);
      return typeof id === "string" && id.indexOf(prefix) === 0;
    } catch (_) { return false; }
  }
  var queueRemoved = 0, itemsRemoved = 0, i, rq, comp;
  try {
    for (i = app.project.renderQueue.numItems; i >= 1; i -= 1) {
      rq = app.project.renderQueue.item(i);
      comp = null;
    ...
    evidence.open("w");
    evidence.write("CLEANED queue=" + queueRemoved + " items=" + itemsRemoved);
    evidence.close();
  } catch (error) {
    try {
      evidence.open("w");
