...
function Resolve-RunningAfterFx {
  ...
  if ($Running.Count -eq 0) {
    throw "Adobe After Effects is not running. The M3 P1/P2 wrapper requires the isolated self-hosted AE process to be running first."
  }
...
try {
  ...
  $Cli = Join-Path $RepoRoot ".tmp\runtime\apps\desktop-host\src\m3-mask-p1-p2-cli.js"
  if (-not (Test-Path $Cli -PathType Leaf)) { throw "Compiled M3 mask P1/P2 CLI not found: $Cli" }

  Write-Host "EditFlow 2.0 M3 mask/Bezier real-AE P1/P2 proof through authenticated CEP transport"
  ...
  Write-Host "The proof creates only temporary compositions/layers/masks and removes the temporary project items before completion."
  ...
  while (-not $NodeProcess.HasExited) {
    ...
    if ($CleanupComplete) {
      if ($null -eq $ResultSeenAt) {
        ...
        Write-Warning "M3 P1/P2 proof and cleanup are complete but Node did not exit; terminating only the completed proof runtime."
    ...
    if ((Get-Date) -ge $HardDeadline) {
      Stop-Process -Id $NodeProcess.Id -Force -ErrorAction SilentlyContinue
      $NodeProcess.WaitForExit()
      throw "M3 mask P1/P2 acceptance exceeded its hard runtime before cleanup completion."
    }
  ...
  if (-not (Test-Path $ResultPath -PathType Leaf)) {
    throw "M3 mask P1/P2 acceptance exited without a proof artifact (exit code $($NodeProcess.ExitCode))."
  }
  ...
  if ($Result.cleanupComplete -ne $true) {
    ...
    throw "M3 P1/P2 result was produced without successful temporary-project cleanup."
  }
  if (-not $Result.ok) {
    $ResultJson | Write-Host
    throw "M3 mask P1/P2 real-AE proof did not pass all bounded checks."
  }
  if (-not $Result.proofLevels.P1_validation_rejection -or -not $Result.proofLevels.P2_structural_readback) {
    throw "M3 result did not truthfully assert both P1 and P2."
  }
  if ($Result.proofLevels.P3_visual_proof -or $Result.proofLevels.P4_failure_injection_rollback -or $Result.proofLevels.P5_save_reopen_reconnect_transfer) {
    throw "M3 P1/P2 harness must not claim P3, P4, or P5."
  }

  Write-Host ("M3 mask P1/P2 proof status: " + $Result.status)
