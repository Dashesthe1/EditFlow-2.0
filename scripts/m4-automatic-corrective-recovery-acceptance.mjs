...
const acceptance = {
  status: accepted ? "PASS_REAL_AE_AUTOMATIC_CORRECTIVE_RECOVERY" : "FAIL_REAL_AE_AUTOMATIC_CORRECTIVE_RECOVERY",
  proofScope: "M4_AUTOMATIC_CORRECTIVE_RECOVERY_DRIFT_FORWARD_REAL_AE",
  accepted,
  sourceCommit,
  ...
  cleanup,
  limitations: [
    "Retained live proof covers persistent drift escalation, exact point-tracker Feature Center correction, and guarded Analyze Forward resume.",
    "Explicit identity-confidence loss uses the same evidence-bound planner and host surface but remains structurally tested rather than independently live-proven.",
...
await writeFile(path.join(root, "proofs", "diagnostics", "m4-automatic-corrective-recovery-real-ae-acceptance.json"), JSON.stringify(acceptance, null, 2), "utf8");
