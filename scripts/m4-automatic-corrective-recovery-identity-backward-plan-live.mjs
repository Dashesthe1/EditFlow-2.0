...
const sample = (timestampMs, identityConfidence, evidenceId) => ({
...
const good = monitor.update(sample(0, 0.99, "ACR:BACKWARD:BASELINE:GOOD"));
const pending = monitor.update(sample(2000, 0.25, "ACR:BACKWARD:IDENTITY:FRAME_2000"));
const escalation = monitor.update(sample(2033, 0.24, "ACR:BACKWARD:IDENTITY:FRAME_2033"));
if (good?.status !== "GOOD" || pending?.status !== "PENDING_FAILURE" || escalation?.status !== "ESCALATE") {
  throw new Error(`automatic backward identity-confidence escalation did not reach ESCALATE: ${JSON.stringify({ good, pending, escalation })}`);
}
...
if (plan.reason !== "IDENTITY_UNCERTAIN" || plan.operations.length !== 3 || plan.analysis.direction !== "BACKWARD") {
