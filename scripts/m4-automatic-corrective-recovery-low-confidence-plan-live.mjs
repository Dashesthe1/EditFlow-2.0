...
const good = monitor.update(sample(0, 0.96, "ACR:BASELINE:GOOD"));
const pending = monitor.update(sample(500, 0.45, "ACR:LOW_CONFIDENCE:FRAME_500"));
const escalation = monitor.update(sample(533, 0.44, "ACR:LOW_CONFIDENCE:FRAME_533"));
if (good?.status !== "GOOD" || pending?.status !== "PENDING_FAILURE" || escalation?.status !== "ESCALATE") {
  throw new Error(`automatic low-confidence escalation did not reach ESCALATE: ${JSON.stringify({ good, pending, escalation })}`);
}
...
if (plan.reason !== "TRACK_CONFIDENCE_LOW" || plan.operations.length !== 3) throw new Error("unexpected automatic corrective plan");
