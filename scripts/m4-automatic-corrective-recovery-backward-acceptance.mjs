...
const checks = {
  persistentDriftEscalated: planResult.monitor?.escalation?.status === "ESCALATE"
    && planResult.monitor?.escalation?.primaryReason === "TRACK_DRIFT_RISK_HIGH",
...
const acceptance = {
  status: accepted ? "PASS_REAL_AE_AUTOMATIC_DRIFT_BACKWARD_CORRECTIVE_RECOVERY" : "FAIL_REAL_AE_AUTOMATIC_DRIFT_BACKWARD_CORRECTIVE_RECOVERY",
  proofScope: "M4_AUTOMATIC_CORRECTIVE_RECOVERY_DRIFT_BACKWARD_REAL_AE",
  accepted,
  sourceCommit,
  ...
  capability: {
    ...
    provedReason: "TRACK_DRIFT_RISK_HIGH",
  ...
  cleanup,
  limitations: [
    "Retained live proof covers persistent drift escalation, exact point-tracker Feature Center correction, and guarded Analyze Backward resume.",
    "All eligible automatic failure reasons are independently live-proven on Analyze Forward; this retained Backward proof isolates the directional host/visual resume path with persistent drift.",
...
await writeFile(path.join(root, "proofs", "diagnostics", "m4-automatic-corrective-recovery-drift-backward-real-ae-acceptance.json"), JSON.stringify(acceptance, null, 2), "utf8");
