...
const acceptance = {
  status: accepted ? "PASS_REAL_AE_AUTOMATIC_IDENTITY_BACKWARD_CORRECTIVE_RECOVERY" : "FAIL_REAL_AE_AUTOMATIC_IDENTITY_BACKWARD_CORRECTIVE_RECOVERY",
  proofScope: "M4_AUTOMATIC_CORRECTIVE_RECOVERY_IDENTITY_BACKWARD_REAL_AE",
  accepted,
  sourceCommit,
  ...
  cleanup,
  limitations: [
    "Retained live proof covers explicit identity-confidence loss escalation, exact point-tracker Feature Center correction, and guarded Analyze Backward resume.",
    "Persistent drift and low tracking confidence use the same evidence-bound Backward host/visual resume path; this proof independently covers explicit identity-confidence loss.",
...
await writeFile(path.join(root, "proofs", "diagnostics", "m4-automatic-corrective-recovery-identity-backward-real-ae-acceptance.json"), JSON.stringify(acceptance, null, 2), "utf8");
