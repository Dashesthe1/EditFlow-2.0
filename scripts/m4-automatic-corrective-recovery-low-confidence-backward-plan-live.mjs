...
const sample = (timestampMs, trackConfidence, evidenceId) => ({
...
const good = monitor.update(sample(0, 0.96, "ACR:BACKWARD:BASELINE:GOOD"));
const pending = monitor.update(sample(2000, 0.45, "ACR:BACKWARD:LOW_CONFIDENCE:FRAME_2000"));
const escalation = monitor.update(sample(2033, 0.44, "ACR:BACKWARD:LOW_CONFIDENCE:FRAME_2033"));
if (good?.status !== "GOOD" || pending?.status !== "PENDING_FAILURE" || escalation?.status !== "ESCALATE") {
  throw new Error(`automatic backward low-confidence escalation did not reach ESCALATE: ${JSON.stringify({ good, pending, escalation })}`);
}
...
if (plan.reason !== "TRACK_CONFIDENCE_LOW" || plan.operations.length !== 3 || plan.analysis.direction !== "BACKWARD") {
