...
const good = monitor.update(sample(0, 0.99, "ACR:BASELINE:GOOD"));
const pending = monitor.update(sample(500, 0.25, "ACR:IDENTITY:FRAME_500"));
const escalation = monitor.update(sample(533, 0.24, "ACR:IDENTITY:FRAME_533"));
if (good?.status !== "GOOD" || pending?.status !== "PENDING_FAILURE" || escalation?.status !== "ESCALATE") {
  throw new Error(`automatic identity-confidence escalation did not reach ESCALATE: ${JSON.stringify({ good, pending, escalation })}`);
}
...
if (plan.reason !== "IDENTITY_UNCERTAIN" || plan.operations.length !== 3) throw new Error("unexpected automatic corrective plan");
