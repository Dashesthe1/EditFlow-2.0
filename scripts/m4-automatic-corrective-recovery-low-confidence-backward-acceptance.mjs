...
const visual = await read("proofs", "artifacts", "m4-automatic-corrective-recovery-backward-visual", "result.json");
const readback = await read("proofs", "artifacts", "m4-automatic-corrective-recovery-backward-readback.json");
const cleanup = await read("proofs", "artifacts", "m4-automatic-corrective-recovery-cleanup.json");
const plan = planResult.plan;
const nearestPre = readback.nearestPreRepairSample ?? null;
if (!planResult.ok || !plan || !nearestPre) throw new Error("Automatic backward corrective recovery proof inputs are incomplete.");

const finite01 = (value) => typeof value === "number" && Number.isFinite(value) && value >= 0 && value <= 1;
const trackConfidence = nearestPre.confidence;
const driftRisk = Math.min(1, Math.max(0, Number(readback.groundTruthErrorPx) / 90));
if (!finite01(trackConfidence) || !finite01(driftRisk)) throw new Error("Protocol 2.1 backward post-analysis confidence/drift evidence is invalid.");
const latestCorrectionMs = Math.max(...plan.stateAfterCorrection.corrections.map((item) => item.timestampMs));
const verificationTimestampMs = Math.max(latestCorrectionMs, planResult.monitor?.escalation?.sample?.timestampMs ?? 0) + 1;
const verified = transitionTrackingRepairV1(plan.stateAfterCorrection, {
  ...
  driftRisk,
  occlusion: 0,
  evidenceIds: ["ACR:PROTOCOL21_BACKWARD_POST_ANALYSIS_SAMPLE", "ACR:BACKWARD_FIXTURE_UNOCCLUDED_GROUND_TRUTH"],
});
const resumed = verified ? transitionTrackingRepairV1(verified, {
  type: "RESUME",
  timestampMs: verificationTimestampMs,
  evidenceIds: ["ACR:GUARDED_ANALYZE_BACKWARD_COMPLETED"],
}) : null;
const completion = visual?.proof?.completion;
const fixture = planResult.fixture;
const exactTarget = plan.operations?.[1]?.payload;
const checks = {
  lowTrackingConfidenceEscalated: planResult.monitor?.escalation?.status === "ESCALATE"
    && planResult.monitor?.escalation?.primaryReason === "TRACK_CONFIDENCE_LOW",
  backwardDirectionComposed: plan.analysis?.direction === "BACKWARD",
  ...
  nativeSamplesGrew: readback?.checks?.keyCountGrew === true && readback?.checks?.preRepairSamplesAdded === true,
  backwardTrajectoryMatchedGroundTruth: readback?.checks?.backwardMotionObserved === true && readback?.checks?.groundTruthTrajectoryMatched === true,
...
const acceptance = {
  status: accepted ? "PASS_REAL_AE_AUTOMATIC_LOW_CONFIDENCE_BACKWARD_CORRECTIVE_RECOVERY" : "FAIL_REAL_AE_AUTOMATIC_LOW_CONFIDENCE_BACKWARD_CORRECTIVE_RECOVERY",
  proofScope: "M4_AUTOMATIC_CORRECTIVE_RECOVERY_LOW_CONFIDENCE_BACKWARD_REAL_AE",
  accepted,
  sourceCommit,
  ...
  capability: {
    ...
    provedReason: "TRACK_CONFIDENCE_LOW",
    provedDirection: "BACKWARD",
  ...
  resume: {
    ...
    nearestPreRepairSample: nearestPre,
  },
  stateVerification: {
    verificationTimestampMs,
    evidenceSampleMediaTime: nearestPre.time,
  ...
  cleanup,
  limitations: [
    "Retained live proof covers persistent low tracking-confidence escalation, exact point-tracker Feature Center correction, and guarded Analyze Backward resume.",
    "Persistent drift and explicit identity-confidence loss use the same evidence-bound Backward host/visual resume path; this proof independently covers low tracking confidence.",
...
await writeFile(path.join(root, "proofs", "diagnostics", "m4-automatic-corrective-recovery-low-confidence-backward-real-ae-acceptance.json"), JSON.stringify(acceptance, null, 2), "utf8");
