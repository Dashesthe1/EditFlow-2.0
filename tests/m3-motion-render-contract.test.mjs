...
import { AE_MOTION_RENDER_ADAPTER_BUILD_V110, AE_MOTION_RENDER_COMMANDS_V110, AE_MOTION_RENDER_PROTOCOL_VERSION_V110, AE_MOTION_RENDER_ROUTE_ID_V110, capabilityForMotionRenderCommandV110 } from "../.tmp/runtime/packages/adapters/ae-cep/src/protocol-v1_10.js";
import { M3_MOTION_RENDER_CAPABILITIES_V110, buildMotionRenderRequestV110 } from "../.tmp/runtime/packages/adapters/ae-cep/src/m3-motion-render.js";
...
test("protocol 1.10 declares the bounded motion/frame rendering surface", () => {
  ...
  assert.deepEqual(AE_MOTION_RENDER_COMMANDS_V110, ["comp.motion_render.set", "layer.motion_render.set", "motion_render.readback"]);
...
test("protocol 1.10 capabilities are FULL at accepted real-AE P5 transfer maturity", () => {
...
test("protocol 1.10 request builder correlates exact command/capability and defaults structural readback", () => {
  const request = buildMotionRenderRequestV110({ requestId: "req-motion-render", transactionId: "tx-motion-render", operationId: "op-motion-render", command: "layer.motion_render.set", expectedHostProjectRevision: 42, payload: { comp: { stableId: "COMP" }, layer: { stableId: "LAYER" }, settings: { motionBlur: true, frameBlendingType: "PIXEL_MOTION" } } });
...
test("motion-render host is revision-gated exact-readback verified and self-restoring on failure", async () => {
  const source = await readFile(hostPath, "utf8");
  for (const pattern of [/EXPECTED_HOST_REVISION_REQUIRED/, /HOST_REVISION_CONFLICT/, /COMP_MOTION_RENDER_READBACK_MISMATCH/, /LAYER_MOTION_RENDER_READBACK_MISMATCH/, /restoreComp\(prepared\.comp, beforeComp\)/, /restoreLayer\(prepared\.layer, beforeLayer\)/, /COMP_MOTION_RENDER_ROLLBACK_MISMATCH/, /LAYER_MOTION_RENDER_ROLLBACK_MISMATCH/]) assert.match(source, pattern);
});

test("protocol 1.10 loader remains additive and fails closed to accepted protocol 1.9", async () => {
