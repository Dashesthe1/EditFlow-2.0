...
export const M4_SEGMENTATION_MATTE_EXPORT_CAPABILITY_V1: CapabilityRecord = {
  ...
  routes: [{
    ...
    limitations: [
      "Planning only; this tranche does not dispatch AE mutations, but its exact protocol 1.3 payload is exercised inside retained deterministic real-AE materialization visual/rollback proof.",
  ...
  limitations: [
    "Any write executor must prove that matteLayer originates from the exact artifactId before dispatching ae.layer.track_matte.set; the retained proof harness satisfies this precondition for its bounded fixture.",
    "Cropped raster bounds require truthful layer placement/alignment before matte binding; the composed static materialization planner has retained real-AE visual evidence for this path.",
    "Runtime capability registration remains withheld pending checkpoint-backed live provider output plus materially different-footage and session-boundary transfer evidence."
  ],
