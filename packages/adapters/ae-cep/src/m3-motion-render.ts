...
import { applyM3MotionRenderAcceptedP5Evidence } from "./m3-motion-render-proof-maturity.js";
import {
...
const ensureMotionRenderResponseV110 = (
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new TypeError("AE motion-render adapter returned a non-object response.");
  const candidate = value as Record<string, unknown>;
  if (candidate["protocolVersion"] !== AE_MOTION_RENDER_PROTOCOL_VERSION_V110) throw new TypeError("AE motion-render adapter protocol version mismatch.");
  if (candidate["requestId"] !== request.requestId || candidate["operationId"] !== request.operationId) throw new TypeError("AE motion-render adapter response correlation mismatch.");
  if (candidate["command"] !== request.command || !isAeMotionRenderCommandV110(String(candidate["command"]))) throw new TypeError("AE motion-render adapter returned an invalid command correlation.");
  const outcome = candidate["outcome"];
  if (outcome !== "APPLIED" && outcome !== "NO_OP" && outcome !== "REJECTED" && outcome !== "FAILED") throw new TypeError("AE motion-render adapter returned an invalid operation outcome.");
...
export class CepEvalScriptMotionRenderTransportV110 {
  ...
  constructor(bridge: CepEvalScriptMotionRenderBridgeV110) { this.bridge = bridge; }
  async dispatch(request: AeMotionRenderRequestV110): Promise<AeMotionRenderResponseV110> {
    ...
    return await new Promise<AeMotionRenderResponseV110>((resolve, reject) => {
      this.bridge.evalScript(script, (rawResult) => {
        try { resolve(ensureMotionRenderResponseV110(JSON.parse(rawResult) as unknown, request)); }
...
const M3_MOTION_RENDER_DECLARED_CAPABILITIES_V110: readonly CapabilityRecord[] = AE_MOTION_RENDER_COMMANDS_V110.map((command): CapabilityRecord => ({
  ...
  routes: [{ routeId: asRouteId(AE_MOTION_RENDER_ROUTE_ID_V110), kind: "HOST_ADAPTER", available: true, adapterVersion: AE_MOTION_RENDER_ADAPTER_BUILD_V110 }],
...
export const M3_MOTION_RENDER_CAPABILITIES_V110: readonly CapabilityRecord[] =
  applyM3MotionRenderAcceptedP5Evidence(M3_MOTION_RENDER_DECLARED_CAPABILITIES_V110);
