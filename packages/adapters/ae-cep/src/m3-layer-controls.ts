...
const ensureLayerControlsResponseV16 = (value: unknown, request: AeLayerControlsRequestV16): AeLayerControlsResponseV16 => {
...
const riskForLayerControlsCommand = (command: AeLayerControlsCommandV16): CapabilityRecord["riskClass"] =>
  command.endsWith(".readback") ? "R0_READ_ONLY" : "R1_REVERSIBLE";

export const M3_LAYER_CONTROLS_CAPABILITIES_V16: readonly CapabilityRecord[] = AE_LAYER_CONTROLS_COMMANDS_V16.map(
  (command): CapabilityRecord => ({
    id: asCapabilityId(capabilityForLayerControlsCommandV16(command)),
    domain: command.startsWith("comp.") ? "comp" : "layer",
    description: `M3 typed AE layer-control command '${command}'. Protocol 1.6 covers script-writable timeline switches and quality controls while composing with the already-proven layer-order route. Motion blur and frame blending remain separate roadmap capabilities.`,
    status: "PARTIAL",
    ...
    rollbackStrategy: command.endsWith(".readback") ? "NONE_REQUIRED" : "AE_UNDO_GROUP",
...
export const buildLayerControlsRequestV16 = (input: {
  ...
  readbackProfile: input.readbackProfile ?? "M3_LAYER_CONTROLS_STRUCTURAL",
});
