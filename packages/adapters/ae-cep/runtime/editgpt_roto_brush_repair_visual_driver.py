...
def locate_layer_canvas(qwen, image, request: dict) -> tuple[tuple[int, int, int, int], dict]:
    target, observation = choose_pointer_target(
        ...
        instruction=(
            ...
            "Exclude gray UI surround, tabs, rulers, timeline, viewer controls, and black or empty letterbox/pillarbox padding; the box must cover only the actual displayed source-image pixels."
        ),
        client=qwen,
        min_confidence=0.65,
    )
    ...
    if width < 180 or height < 120 or width * height < image.shape[0] * image.shape[1] * 0.08:
