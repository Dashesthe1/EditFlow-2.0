...
M4 now advances through point tracking, two-point rotation/scale tracking, four-point and perspective tracking, mask tracking, face tracking, stabilization, semantic attach points, subject/object segmentation, segmentation-to-mask/matte export, and manual repair/resume. Retained real-AE evidence now covers point tracking, guarded native analysis, two-point transform derivation, four-point perspective/homography derivation, native Position stabilization, and protocol 2.4 Feature Center repair followed by guarded Analyze Forward and Analyze Backward resume with independent protocol 2.1 verification. A fail-closed local SAM 3.1 segmentation-provider foundation now enforces exact source/provenance binding, explicit SAM 3.1 checkpoint selection, and byte-verified mask artifacts, but it remains unregistered pending live model inference evidence; the selected development venv currently lacks the `sam3` package and authenticated gated checkpoint access. Mask-point repair, automatic drift/identity-loss recovery, and segmentation materialization/application remain open M4 work. Development continues on the persistent warm After Effects process so routine proof cycles stay around 30 seconds or faster wherever the operation itself allows it.
