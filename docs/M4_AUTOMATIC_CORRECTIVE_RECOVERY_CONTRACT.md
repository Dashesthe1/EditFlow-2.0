...
Protocol 2.1 post-analysis truth increased keyed samples from 2 to 3, preserved the repair key, measured the first native directional sample at 0.998654 confidence and approximately 0.1495 px ground-truth error, satisfied `VERIFY_REPAIR`, reached `RESUMED`, restored the project from the proof fixture back to its 4-item baseline, and preserved the same warm After Effects process. The retained Backward proofs start from a later repair frame at 2.0 seconds, correct `[470,330]` to `[444,360]`, and verify the new earlier native sample at approximately 1.9667 seconds; low-confidence, drift, and explicit identity-loss triggers each pass this same exact host/visual verification path independently.

All three V1-eligible automatic failure reasons are independently live-proven on the same exact host correction surface for both Analyze Forward and Analyze Backward. Subject occlusion remains escalation-only because hidden geometry is not safe automatic-write authority. Unit coverage continues to enforce fail-closed identity, mapping, timing, direction, and point-binding behavior.
