...
All three V1-eligible automatic failure reasons are now independently live-proven on the same exact host correction surface for Analyze Forward. Automatic Analyze Backward remains structurally composed rather than independently live-proven. Subject occlusion remains escalation-only. Unit coverage continues to enforce fail-closed identity, mapping, timing, and point-binding behavior.
