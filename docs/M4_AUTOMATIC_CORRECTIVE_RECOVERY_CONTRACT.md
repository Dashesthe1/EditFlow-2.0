...
Current maturity is `VISUAL` for independently retained persistent-low-confidence, persistent-drift, and explicit identity-confidence-loss / Analyze Forward routes. Each warm real-AE proof observed `GOOD -> PENDING_FAILURE -> ESCALATE`, consumed the composer's exact three-operation protocol 2.4 plan, observed the deliberately wrong `[250,330]` Feature Center before mutation, corrected it to `[276,360]` with exact independent post-readback, then completed guarded Analyze Forward through EditGPT Eyes/Hands. The low-confidence proof isolates the cause by holding drift risk at `0.04`, occlusion at `0`, and identity confidence at `0.99` while tracking confidence falls to `0.45` then `0.44`; the identity proof holds tracking confidence at `0.96` while identity confidence falls to `0.25` then `0.24`.

Protocol 2.1 post-analysis truth increased keyed samples from 2 to 3, preserved the repair key, measured the first native directional sample at 0.998654 confidence and approximately 0.1495 px ground-truth error, satisfied `VERIFY_REPAIR`, reached `RESUMED`, restored the project from the proof fixture back to its 4-item baseline, and preserved the same warm After Effects process. The retained drift/Backward proof starts from a later repair frame at 2.0 seconds, corrects `[470,330]` to `[444,360]`, and verifies the new earlier native sample at approximately 1.9667 seconds.

All three V1-eligible automatic failure reasons are independently live-proven on the same exact host correction surface for Analyze Forward. Persistent drift is additionally live-proven through the composed Analyze Backward route using the same exact correction and post-analysis gates. Low-confidence and identity-loss Backward combinations remain structurally composed rather than independently live-proven. Subject occlusion remains escalation-only. Unit coverage continues to enforce fail-closed identity, mapping, timing, direction, and point-binding behavior.
