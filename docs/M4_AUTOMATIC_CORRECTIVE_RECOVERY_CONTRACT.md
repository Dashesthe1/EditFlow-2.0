...
Current maturity is `VISUAL` for independently retained persistent-drift and explicit identity-confidence-loss / Analyze Forward routes. Each warm real-AE proof observed `GOOD -> PENDING_FAILURE -> ESCALATE`, consumed the composer's exact three-operation protocol 2.4 plan, observed the deliberately wrong `[250,330]` Feature Center before mutation, corrected it to `[276,360]` with exact independent post-readback, then completed guarded Analyze Forward through EditGPT Eyes/Hands. The identity proof isolates the cause by holding tracking confidence at `0.96`, drift risk at `0.04`, and occlusion at `0` while identity confidence falls to `0.25` then `0.24`.
...
Low tracking-confidence correction remains structurally tested over the same exact host correction surface rather than independently live-proven. Subject occlusion remains escalation-only. Unit coverage continues to enforce fail-closed identity, mapping, timing, and point-binding behavior.
