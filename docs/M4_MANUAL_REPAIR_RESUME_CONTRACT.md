...
The deterministic repair/resume model is now connected to retained real-AE **point-tracker Feature Center repair + Analyze Forward/Backward resume** paths. Automatic confidence/drift/occlusion/explicit-identity escalation remains structurally modeled, and the new corrective composer now has retained warm real-AE acceptance for the persistent-drift / Analyze Forward path: automatic escalation, exact protocol 2.4 correction, exact post-readback, guarded visual resume, protocol 2.1 trajectory verification, state-machine `RESUMED`, and baseline cleanup all pass in the same AE process. Mask-point repair also has retained warm real-AE acceptance for exact static-path vertex correction through protocol 1.2, including post-write readback, typed Undo restoration, reapply, viewer-visible correction, and cleanup. Human-parity remains partial: tangent/animated mask-point repair, an independently retained live identity-loss corrective proof, occlusion recovery, and generalized semantic correction-to-host mapping still require separate retained proofs.
