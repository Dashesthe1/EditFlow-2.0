...
3. retain independent shot-level truth for full-length real movies and difficult references,
   ...
   and now requires at least 20 passing cases with 20 distinct Finish reference IDs and 20 distinct
   Finish media SHA-256 identities before suite-level generalization can be certified.
...
human grading without weakening the machine mastery gate.
