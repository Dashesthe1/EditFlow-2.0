...
These controllers and visual drivers remain proof-only and are not registered in the default desktop runtime. Refine Edge, guarded freeze/unfreeze, guarded manual repair, structural `TRACK_MATTE` export, and materially different-footage transfer now have retained development proof; `MASK` conversion, explicit popup fault injection, and production route registration remain unproven.
...
The visual driver advertises both proven seed roles but remains unregistered in the default desktop runtime. With structural `TRACK_MATTE` export and materially different-footage transfer now accepted for development proof, production promotion still requires explicit popup-fault handling and production registration. `MASK` conversion remains an unsupported export variant rather than an implied fallback.
