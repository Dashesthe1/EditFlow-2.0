...
## Proof-only EditGPT foreground seed driver

`EditGptRotoBrushSeedVisualDriverV1` now wraps a fixed EditGPT Eyes/Hands sidecar for `SEED_FOREGROUND` development proof. It correlates the exact comp/layer binding, session revision, effect-match count, time, stroke geometry, tool identity, and retained evidence IDs. The sidecar verifies the bound Layer viewer, grounds the visible layer canvas, maps normalized layer-space stroke points only inside that verified canvas, selects the Roto Brush tool, and issues one guarded drag. Post-action modal errors are inspected; only an unambiguous single OK/Close acknowledgement may be dismissed, with the visible error text retained in the refusal detail. Actual AE action-to-action latency is returned as evidence.

The driver advertises only `FOREGROUND`. `SEED_BACKGROUND` remains fail-closed because the current Hands drag primitive does not yet prove a modifier-held drag suitable for the Roto Brush subtract/background gesture. The visual driver is not registered in the default desktop runtime. Live promotion still requires the isolation preflight, proof-owned fixture setup, protocol-2.6 pre/post readback, changed native effect fingerprint, cleanup/checkpoint restoration, popup proof, and later transfer to materially different footage.
...
Restore is mandatory in `finally`. It may discard only an empty unsaved project or an unsaved project whose every item is explicitly proof-owned with the `EF2_M5_ROTO_` prefix. Any unexpected saved project or foreign unsaved item fails closed. The exact original project must reopen with its original item count, clean state, and active item, while the After Effects process identity remains unchanged. This isolation contract does not itself prove Roto Brush mutation or production readiness; it only establishes a reversible environment in which the later protocol-2.6 + guarded visual seed proof may run.
