...
Accepted 2026-09-16: Mocha AE discovery, exact effect application, and guarded session launch are grounded against the live warm host. AE reports display name `Mocha AE`, match name `mochaAECC`, category `Boris FX Mocha`, version `12.2`; a proof-owned isolation run adds exactly one effect, captures a 62-node structural tree including `Launch Mocha AE` / `mochaAECC-2353`, and the launch gate verifies one responsive Boris FX `mocha4ae_adobe.exe` / `Mocha AE` window before proof-owned cleanup and exact saved-project restoration on the same AE PID. The hardened launch gate handles the exact first-run `Registration` modal only through `Register later`, verifies the unobstructed Mocha workspace, and recorded a maximum warm AE roundtrip of approximately 1197.3 ms. Guarded click-to-responsive-Mocha-window verification was approximately 5.64 s and is tracked separately from warm AE action latency. Planar-region creation, tracking, repair, and export are not yet accepted. See docs/M5_MOCHA_AE_ADAPTER_CONTRACT.md.
