...
Accepted 2026-09-16: Mocha AE discovery, exact effect application, and guarded session launch are grounded against the live warm host. AE reports display name `Mocha AE`, match name `mochaAECC`, category `Boris FX Mocha`, version `12.2`; a proof-owned isolation run adds exactly one effect, captures a 62-node structural tree including `Launch Mocha AE` / `mochaAECC-2353`, and the launch gate verifies one responsive Boris FX `mocha4ae_adobe.exe` / `Mocha AE` window before proof-owned cleanup and exact saved-project restoration on the same AE PID. The hardened launch gate handles the exact first-run `Registration` modal only through `Register later`, verifies the unobstructed Mocha workspace, and recorded a maximum warm AE roundtrip of approximately 1197.3 ms. Guarded click-to-responsive-Mocha-window verification was approximately 5.64 s and is tracked separately from warm AE action latency. Planar-region creation is now accepted: the retained gate clears only the fixed known startup prompts, binds the exact `Create X-spline Layer` tool, creates one explicit bounded four-point region, independently verifies exactly one Mocha `Layer 1`, closes the proof-owned session, restores revision `126` on the same AE process, and records a 1220.155 ms maximum warm AE roundtrip. Bounded planar tracking is now accepted: the retained gate seeds exact frame `1`, invokes only the exact single-frame forward/backward tracking controls, independently resolves tracked endpoints `2` and `0`, restores revision `126` cleanly on the same AE process, records a `1208.451 ms` maximum warm AE roundtrip, and reports a `551.672 ms` maximum one-frame Mocha solve separately. Repair and export remain unaccepted. See docs/M5_MOCHA_AE_ADAPTER_CONTRACT.md.
