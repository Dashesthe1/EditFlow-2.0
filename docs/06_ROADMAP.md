...
In progress 2026-09-16: the Roto Brush / Refine Edge adapter now has a declared semantic contract plus accepted retained real-AE foreground seed, background seed, bounded forward/backward propagation, Refine Edge, guarded freeze/unfreeze, guarded manual repair, and structural TRACK_MATTE export proofs. The seed/refine proofs establish exact protocol-2.6 target/effect/stroke readback; propagation establishes bounded symmetric frame stepping; freeze establishes visible FREEZE -> FROZEN -> UNFREEZE -> UNFROZEN truth; repair visibly corrects a known matte defect with native stroke readback; and TRACK_MATTE export duplicates the exact isolated layer through the typed v1.1 host route while preserving stable identity, layer structure, and the native Roto effect fingerprint. These proofs preserve one warm AE process and exact project restore. The accepted export proof measured a maximum warm AE action gap of approximately 261.6 ms and a typed duplicate roundtrip of approximately 203.4 ms, with all measured actions sub-second. Runtime registration remains intentionally unavailable until popup-fault, materially different-footage transfer, and production-registration gates pass. See docs/M5_ROTO_BRUSH_ADAPTER_CONTRACT.md.
