...
In progress 2026-09-16: the Roto Brush / Refine Edge adapter now has a declared semantic contract plus accepted retained real-AE foreground seed, background seed, bounded forward/backward propagation, Refine Edge, guarded freeze/unfreeze, guarded manual repair, structural TRACK_MATTE export, materially different-footage transfer, and popup-fault recovery proofs. The transfer gate repeats the accepted guarded seed + typed TRACK_MATTE export on 1080x1080 ~59.97 fps footage and 3840x1600 ~23.976 fps footage with distinct source digests, stable output ID, exact native Roto fingerprint preservation, same-process restore, and a maximum warm AE action gap of approximately 259.2 ms. The popup-fault gate captures the exact AE script error, permits only the declared matching acknowledgement, preserves the same AE PID, and proves a post-fault health-script roundtrip of approximately 1837.0 ms without restart or retry. Runtime registration remains intentionally unavailable until the production-registration gate passes. See docs/M5_ROTO_BRUSH_ADAPTER_CONTRACT.md.
