...
In progress 2026-09-16: the Roto Brush / Refine Edge adapter now has a declared semantic contract plus accepted retained real-AE foreground seed, background seed, bounded forward/backward propagation, Refine Edge, guarded freeze/unfreeze, guarded manual repair, structural TRACK_MATTE export, and materially different-footage transfer proofs. The transfer gate repeats the accepted guarded seed + typed TRACK_MATTE export on 1080x1080 ~59.97 fps footage and 3840x1600 ~23.976 fps footage with distinct source digests, stable output ID, exact native Roto fingerprint preservation, same-process restore, and a maximum warm AE action gap of approximately 259.2 ms. Runtime registration remains intentionally unavailable until popup-fault handling and production-registration gates pass. See docs/M5_ROTO_BRUSH_ADAPTER_CONTRACT.md.
