...
Accepted 2026-09-16: Mocha AE discovery is grounded read-only against the live warm host. AE reports display name `Mocha AE`, match name `mochaAECC`, category `Boris FX Mocha`, version `12.2`, with the effect addable to the current AV layer; the same AE PID and unchanged project are preserved. Mutation/launch/tracking/export are not yet accepted. See docs/M5_MOCHA_AE_ADAPTER_CONTRACT.md.
