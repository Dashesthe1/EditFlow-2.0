...
In progress 2026-09-16: the Roto Brush / Refine Edge adapter now has a declared semantic contract plus accepted retained real-AE foreground seed, background seed, bounded forward/backward propagation, Refine Edge, and guarded freeze/unfreeze proofs. The seed/refine proofs establish exact protocol-2.6 target/effect/stroke readback; propagation establishes bounded symmetric frame stepping; and the freeze proof establishes visible FREEZE -> FROZEN -> UNFREEZE -> UNFROZEN truth while preserving one warm AE process and exact project/native-effect restore. The latest warm action gap was approximately 243 ms, comfortably below the 3-second ceiling. Runtime registration remains intentionally unavailable until repair/export, popup-fault, transfer, and production-registration gates pass. See docs/M5_ROTO_BRUSH_ADAPTER_CONTRACT.md.
