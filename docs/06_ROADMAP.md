...
In progress 2026-09-16: the Roto Brush / Refine Edge adapter now has a declared semantic contract plus accepted retained real-AE foreground seed, background seed, bounded forward/backward propagation, Refine Edge, guarded freeze/unfreeze, and guarded manual repair proofs. The seed/refine proofs establish exact protocol-2.6 target/effect/stroke readback; propagation establishes bounded symmetric frame stepping; freeze establishes visible FREEZE -> FROZEN -> UNFREEZE -> UNFROZEN truth; and repair visibly corrects a known matte defect with native stroke readback. These proofs preserve one warm AE process and exact project restore. The accepted repair proof measured a maximum warm AE action gap of approximately 254.8 ms, comfortably below the 3-second ceiling, with all measured action gaps sub-second. Runtime registration remains intentionally unavailable until export, popup-fault, transfer, and production-registration gates pass. See docs/M5_ROTO_BRUSH_ADAPTER_CONTRACT.md.
