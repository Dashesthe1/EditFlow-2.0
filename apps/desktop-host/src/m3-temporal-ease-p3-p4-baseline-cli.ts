...
import { AE_TEMPORAL_EASE_PROTOCOL_VERSION_V18 } from "../../../packages/adapters/ae-cep/src/protocol-v1_8.js";
...
interface BaselineFile {
  readonly schemaVersion: 1;
  readonly proofId: "M3_TEMPORAL_EASE_P3_P4_BASELINE";
...
const parseConfig = (value: unknown): BridgeConfigFile => {
  ...
  if (!Array.isArray(supported)
    ...
    throw new Error("Bridge config does not advertise required 1.8 and 1.1 protocols.");
  }
...
const parseBaseline = (value: unknown): BaselineFile => {
  ...
  const candidate = value as Record<string, unknown>;
  if (candidate["schemaVersion"] !== 1 || candidate["proofId"] !== "M3_TEMPORAL_EASE_P3_P4_BASELINE") throw new Error("Baseline file identity is invalid.");
...
const requestIdFactory = (() => {
  ...
  return (): string => {
    sequence += 1;
    return `m3-temporal-ease-p34-baseline-${Date.now()}-${sequence}`;
  };
})();
...
const main = async (): Promise<void> => {
  ...
  const broker = new LoopbackCepBroker({
    ...
    supportedProtocolVersions: [AE_TEMPORAL_EASE_PROTOCOL_VERSION_V18, AE_ADAPTER_PROTOCOL_VERSION_V11],
  });
  ...
  try {
    ...
    if (panel.protocolVersion !== AE_TEMPORAL_EASE_PROTOCOL_VERSION_V18
        || !panel.supportedProtocolVersions.includes(AE_ADAPTER_PROTOCOL_VERSION_V11)) {
      throw new Error("Baseline verification requires negotiated protocol 1.8 with protocol 1.1 compatibility.");
    }
    ...
    const observed = await client.observe("m3-temporal-ease-p3-p4-baseline");
    const baseline: BaselineFile = {
      schemaVersion: 1,
      proofId: "M3_TEMPORAL_EASE_P3_P4_BASELINE",
    ...
    if (expectedPath === null) {
      if (baseline.itemCount !== 0 || baseline.filePath !== null) throw new Error("Temporal-ease P3/P4 baseline must be a blank unsaved project.");
