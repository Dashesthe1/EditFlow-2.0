...
const parseConfig = (value: unknown): BridgeConfigFile => {
  ...
  if (!Array.isArray(supported)
    ...
    throw new Error("CEP bridge config does not advertise both required layer-controls 1.6 and baseline 1.1 protocols.");
  }
...
const layerSwitchRecord = (response: AeLayerControlsResponseV16): Record<string, unknown> | null =>
  nestedRecord(nestedRecord(response.readback, "layerSwitches"), "switches");
...
const main = async (): Promise<void> => {
  ...
  const sourcePath = path.join(artifactDir, "p3-layer-control-source.bmp");
  const enabledRenderPath = path.join(artifactDir, "p3-enabled-visible.avi");
  const disabledRenderPath = path.join(artifactDir, "p3-disabled-hidden.avi");
  const restoredRenderPath = path.join(artifactDir, "p3-restored-visible.avi");
  const recoveryRenderPath = path.join(artifactDir, "p4-post-rollback-visible.avi");
  ...
  let enabledArtifactPath: string | null = null;
  let disabledArtifactPath: string | null = null;
  let restoredArtifactPath: string | null = null;
  ...
  const temporaryItemStableIds = new Set([sourceStable, targetCompStable]);
  ...
  try {
    ...
    await writeFile(sourcePath, createBmp24(320, 320, (x, y) => {
      if (x < 160 && y < 160) return [232, 64, 64];
      if (x >= 160 && y < 160) return [64, 216, 96];
      if (x < 160) return [64, 112, 232];
      return [232, 200, 64];
    }));
    ...
    const imported = await executeV11("media.import", { path: sourcePath, stableId: sourceStable, sequence: false });
    checks.source_import = imported.affectedObjects.some((item) => item.stableId === sourceStable && item.kind === "FOOTAGE");
    const target = await executeV11("comp.create", {
      stableId: targetCompStable,
      name: `${prefix} Switch Visibility`,
      width: 320,
      height: 320,
      pixelAspect: 1,
      duration: 1,
    ...
    checks.target_layer_create = nestedRecord(layer.readback, "layer")?.["stableId"] === layerStable;

    const initialReadback = await dispatchV16("layer.switches_readback", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable },
    }, null);
    const initialSwitches = layerSwitchRecord(initialReadback);
    if (initialSwitches === null) throw new Error("Initial layer switch readback is missing.");
    checks.p3_initial_enabled = initialReadback.outcome === "NO_OP"
      && initialSwitches["enabled"] === true
      && initialSwitches["solo"] === false
      && initialSwitches["shy"] === false;
    const initialSwitchesJson = stableJson(initialSwitches);

    const enabledCompletion = await renderComp(enabledRenderPath);
    enabledArtifactPath = enabledCompletion.outputPath;
    checks.p3_enabled_visual_artifact_emitted = enabledCompletion.ok && await fileExistsNonEmpty(enabledArtifactPath);

    const disable = await dispatchV16("layer.switches.set", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable }, switches: { enabled: false },
    }, hostRevision);
    checks.p3_disable_applied = disable.outcome === "APPLIED" && layerSwitchRecord(disable)?.["enabled"] === false;
    await refreshState();
    const disabledCompletion = await renderComp(disabledRenderPath);
    disabledArtifactPath = disabledCompletion.outputPath;
    checks.p3_disabled_visual_artifact_emitted = disabledCompletion.ok && await fileExistsNonEmpty(disabledArtifactPath);

    const restore = await dispatchV16("layer.switches.set", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable }, switches: { enabled: true },
    }, hostRevision);
    checks.p3_restore_applied = restore.outcome === "APPLIED" && layerSwitchRecord(restore)?.["enabled"] === true;
    await refreshState();
    const restoredCompletion = await renderComp(restoredRenderPath);
    restoredArtifactPath = restoredCompletion.outputPath;
    checks.p3_restored_visual_artifact_emitted = restoredCompletion.ok && await fileExistsNonEmpty(restoredArtifactPath);
    checks.p3_visual_artifact_emitted = checks.p3_enabled_visual_artifact_emitted
      && checks.p3_disabled_visual_artifact_emitted
      && checks.p3_restored_visual_artifact_emitted;

    const beforeFailureReadback = await dispatchV16("layer.switches_readback", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable },
    }, null);
    const beforeFailureSwitches = layerSwitchRecord(beforeFailureReadback);
    if (beforeFailureReadback.outcome !== "NO_OP" || beforeFailureSwitches === null) throw new Error("P4 baseline switch readback failed.");
    checks.p4_baseline_matches_initial = stableJson(beforeFailureSwitches) === initialSwitchesJson;
    ...
    const inducedFailure = await dispatchV16("layer.switches.set", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable }, switches: { shy: true },
    ...
    checks.p4_self_rollback_note = inducedFailure.diagnostics.notes.includes("Failed layer-switch mutation self-rolled back with AE Undo.");
    ...
    const afterFailureReadback = await dispatchV16("layer.switches_readback", {
      comp: { stableId: targetCompStable }, layer: { stableId: layerStable },
    }, null);
    const afterFailureSwitches = layerSwitchRecord(afterFailureReadback);
    checks.p4_switch_state_restored = afterFailureReadback.outcome === "NO_OP"
      && afterFailureSwitches !== null
      && stableJson(afterFailureSwitches) === stableJson(beforeFailureSwitches);
    ...
    const ok = failureError === null
      ...
      && cleanupComplete
      && checks.fixture_image_written === true
      && checks.panel_negotiated_v16 === true
      && checks.panel_supports_v11_v16 === true
      && checks.host_probe === true
      && checks.source_import === true
      && checks.target_comp_create === true
      && checks.target_layer_create === true
      && checks.p3_initial_enabled === true
      && checks.p3_disable_applied === true
      && checks.p3_restore_applied === true
    ...
    await writeJson(resultPath, {
      ...
      acceptedBaseline: {
        P1_P2_source_commit: "60fd64c67161f9799c3a123570625049c00330d5",
        P1_P2_run: 34145893380,
        P1_P2_attempt: 1,
        P1_P2_artifact: 10027648642,
      },
      proofLevels: {
      ...
      visualReviewSpec: {
        source: sourcePath,
        enabledVisibleRender: enabledArtifactPath,
        disabledHiddenRender: disabledArtifactPath,
        restoredVisibleRender: restoredArtifactPath,
        postRollbackRender: recoveryArtifactPath,
        expected: [
          "The enabled render must show the four-color quadrant source filling the 320x320 composition.",
          "The disabled render must contain none of the four-color source because the only layer is disabled.",
          "The restored render must visually match the enabled render after enabled:true is reapplied.",
          "The post-rollback render must visually match the enabled/restored renders because the induced shy:true mutation is undone.",
          "No source replacement, layer duplication, ordering change, or alternate route is permitted to explain the visibility change.",
        ],
      },
      environment: {
        host: panelHostName,
      ...
      cleanupComplete,
      notes: [
        "P1/P2 are accepted baseline evidence and are not replayed in this P3/P4 tranche.",
        "P3 emits deterministic real-AE enabled/disabled/restored visual artifacts but does not self-claim visual acceptance; retained renders must be independently reviewed before VISUAL maturity is advanced.",
        "P4 is induced only when the runner-owned AE process inherits EDITFLOW_M3_LAYER_CONTROLS_P4_PROOF=1 and a typed layer.switches.set request uses the exact M3_LAYER_CONTROLS_P4_FAILURE_INJECTION profile.",
        "The P4 error occurs after a real shy:true mutation inside the normal layer-switch undo group; the existing catch path must report AE Undo recovery and restore both project fingerprint and exact switch readback.",
